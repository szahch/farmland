define(
		[ "dojo/_base/declare", "dojo/_base/array", "dojo/query",
				...
				"dojo/_base/lang", "/Farmland/js/map/_Widget.js",
				"dojo/text!./templates/_BaseWidget.html" ],
		function(declare, array, query, domAttr, domStyle, on, lang, _Widget,
				template) {
			return declare(
					_Widget,
					{
						...
						postCreate : function() {
							...
							this.panels.forEach(function(item, idx, arr) {
								item.buttonIcon = domAttr.get(item,
										"buttonIcon");
								item.buttonText = domAttr.get(item,
										"buttonText");
							});
							this.showPanel(0);
						},
						...
						startup : function() {
							...
							if (frame
									&& frame.declaredClass === "webgis2book.widgets.WidgetFrame") {
						...
						getAllNamedChildDijits : function() {
							// 获得所有的子小部件
							var w = query("[widgetId]", this.containerNode
									|| this.domNode);
							var children = w.map(dijit.byNode);

							this.widgets = {};
							children.forEach(lang.hitch(this, function(item,
									idx) {
								if (item.name) {
